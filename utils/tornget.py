...
def tornget(endpoint, key, tots=0, fromts=0):
    url = f'https://api.torn.com/{endpoint}&key={key}&comment=TC{"" if fromts == 0 else f"&from={fromts}"}' \
    ...
    if request.status_code != 200:
        utils.get_logger().warning(f'The Torn API has responded with status code {request.status_code} to endpoint '
                                   f'"{endpoint}".')
        raise utils.NetworkingError(f'The Torn API has responded with status code {request.status_code}.',
                                    request.status_code)
